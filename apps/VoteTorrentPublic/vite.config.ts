...
//
// Second binding rule, this app's own: `resolve.dedupe` must contain exactly `react` and
// `react-dom` and must never be deleted "because the build is green" — the build is green
// in every broken variant; the identity gate 53-09 adds is the only thing that can see its
// absence.
//
