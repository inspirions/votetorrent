...
describe('no-portal / no-vendor regression (PUB-01 / PUB-02)', () => {
  ...
  // PUB-01-c — the 6 packages resolve to their published version lines.
  it('PUB-01-c: the 6 de-vendored packages resolve to published versions (@serfab 0.10.x, @optimystic/db-* 0.22.x)', () => {
    for (const pkg of DEVENDORED_PACKAGES) {
      ...
      const expectedPrefix = pkg.startsWith('@serfab/') ? '0.10.' : '0.22.'
      expect(
        distinct[0],
