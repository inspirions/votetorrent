 ...
 *            1.0.0-beta.x version (off the formerly-patched 0.13.5), and the two
...
describe('published stack lock regression (UPG-06 / PUB-01 / PUB-02)', () => {
  ...
  describe('PUB-02: @optimystic/quereus-plugin-optimystic reconciled to ^1.0.0-beta.x, dead patches retired', () => {
    it('resolves a single @optimystic/quereus-plugin-optimystic version, and it is 1.0.0-beta.x', () => {
      ...
      expect(
        distinct[0],
        `Resolved @optimystic/quereus-plugin-optimystic version must be 1.0.0-beta.x, got ${distinct[0]}`
      ).to.match(/^1\.0\.0-beta\./)
    })
